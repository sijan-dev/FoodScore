...
class GoalHighlighter {
          ...
          if (n.energyKcal > 0 && n.energyKcal <= 200) {
            result.add(
              const GoalHighlight(
                ...
                description:
                    'Low calorie density — fits weight management goals.',
              ),
            );
          }
    ...
    for (final goal in goals) {
      switch (goal) {
        ...
        case 'Dietary Monitoring':
          if (product.additives.isEmpty) {
            adj += 4;
          } else if (product.additives.length > 3) {
            adj -= 3;
          }
          break;
        case 'General Wellness':
    ...
    for (final goal in goals) {
      ...
      switch (goal) {
        ...
        case 'Muscle & Fitness':
          match = _takeFirstMatching(
            remaining,
            (i) => i.toLowerCase().contains('protein'),
          );
        case 'Heart Health':
          match = _takeFirstMatching(
            remaining,
        ...
        case 'Low Carb':
          match = _takeFirstMatching(
            remaining,
            (i) => i.toLowerCase().contains('sugar'),
          );
        case 'Dietary Monitoring':
          match = _takeFirstMatching(
            remaining,
  ...
  static String? _takeFirstMatching(
    List<String> list,
